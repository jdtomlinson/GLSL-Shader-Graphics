	...
	, m_size( 0, 0 )
{
	...
	// Enable texturing
	glRenderer.EnableTexture2D();

	// Tell OpenGL that our pixel data is single-byte aligned
	glRenderer.PixelStore();

	// Ask OpenGL for an unused texName (ID number) to use for this texture
	glRenderer.GenerateTextures( &m_openglTextureID );

	// Tell OpenGL to bind (set) this as the currently active texture
	glRenderer.BindTexture2D( m_openglTextureID );

	// Set texture clamp vs. wrap (repeat)
	glRenderer.SetTexture2DWrapS( REPEAT ); // GL_CLAMP or GL_REPEAT
	glRenderer.SetTexture2DWrapT( REPEAT ); // GL_CLAMP or GL_REPEAT

	// Set magnification (texel > pixel) and minification (texel < pixel) filters
	glRenderer.SetTexture2DMagnificationFilter( LINEAR ); // one of: GL_NEAREST, GL_LINEAR, GL_NEAREST_MIPMAP_NEAREST, GL_NEAREST_MIPMAP_LINEAR, GL_LINEAR_MIPMAP_NEAREST, GL_LINEAR_MIPMAP_LINEAR
	glRenderer.SetTexture2DMinificationFilter( LINEAR_MIPMAP_LINEAR ); // one of: GL_NEAREST, GL_LINEAR, GL_NEAREST_MIPMAP_NEAREST, GL_NEAREST_MIPMAP_LINEAR, GL_LINEAR_MIPMAP_NEAREST, GL_LINEAR_MIPMAP_LINEAR

	glRenderer.SetTexture2DMaxLevel( 5 );
	...
	glRenderer.SetTexture2DImage(	// Upload this pixel data to our new OpenGL texture
	...
	glRenderer.GenerateMipmapHint();
	glRenderer.GenerateMipmapTexture2D();
